...
// State
// ---------------------------------------------------------------------------
interface Checkpoint { step: number; loss: number; trajectory: number[][][]; scoreField: number[][][]; }

const state = {
  dataset: 'moons' as DatasetType,
  arch: { hiddenLayers: 2, hiddenWidth: 64, activation: 'swish' as Activation, timeEmbedDim: 16 } as ArchConfig,
  hyper: {
    T: 40, lr: 0.01, batchSize: 64, totalSteps: 1500,
    checkpointInterval: 40, numTrackedPoints: 350, datasetSize: 4096, betaEnd: 0.02,
  } as HyperParams,
  trueData: [] as number[][],
...
app.innerHTML = `
  <aside class="sidebar">
    ...
    <div>
      ...
      <div class="field">
        <div class="row-between"><label>Noise variance (&beta;<sub>T</sub>)</label></div>
  ...
  <main class="main">
    <div class="panel">
      ...
      <div class="scrubber-row" style="margin-top: 8px; gap: 10px;">
    ...
    <div class="panel">
      <div class="panel-header"><h2>Training loss</h2></div>
...
// ---------------------------------------------------------------------------
// Pill groups (dataset + activation pickers)
// ---------------------------------------------------------------------------
...
betaEndSelect.onchange = () => { state.hyper.betaEnd = Number(betaEndSelect.value); resetRun(); };
...
function attachWorkerHandlers() {
  worker.onmessage = (e: MessageEvent<WorkerOutbound>) => {
    ...
    if (msg.type === 'ready') {
      ...
      state.checkpoints.push({ step: msg.step, loss: msg.loss, trajectory: msg.trajectory, scoreField: msg.scoreField });
...
sceneCanvas.onclick = (e: MouseEvent) => {
  ...
  if (!ckpt) return;
  const points = ckpt.trajectory[Math.min(state.diffusionIdx, ckpt.trajectory.length - 1)];
...
function redrawScene() {
  const ckpt = state.checkpoints[state.trainingIdx];
  const idx = ckpt ? Math.min(state.diffusionIdx, ckpt.trajectory.length - 1) : 0;
  const points = ckpt ? ckpt.trajectory[idx] : null;
  ...
  renderer.draw({
    ...
    trailTrajectory: ckpt ? ckpt.trajectory : null,
    trailUpToIndex: idx,
...
function redrawLoss() {
  const history = state.checkpoints.map(c => ({ step: c.step, loss: c.loss }));
  lossChart.draw(history, state.checkpoints.length > 0 ? state.trainingIdx : null);
}

window.addEventListener('resize', () => { redrawScene(); redrawLoss(); });
