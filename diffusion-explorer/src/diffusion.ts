...
import { VIEW_RANGE, SCORE_GRID_RES, GRAD_CLIP_NORM } from './constants';
...
export function makeSchedule(T: number, betaStart = 1e-4, betaEnd = 0.02, type: ScheduleType = 'linear'): Schedule {
  if (type === 'cosine') {
    // Nichol & Dhariwal (2021) cosine schedule: derive alphaBar_t from a
    // cosine curve, then back out the per-step betas from consecutive
    // alphaBars. This is what almost every modern diffusion model actually
    // uses instead of a linear beta schedule.
    const s = 0.008;
    const fOfT = (t: number) => Math.cos(((t / T + s) / (1 + s)) * (Math.PI / 2)) ** 2;
    const f0 = fOfT(0);
    const betas: number[] = [];
    let prevAlphaBar = 1; // alphaBar at t=0 (no noise yet)
    for (let t = 1; t <= T; t++) {
      const alphaBarT = fOfT(t) / f0;
      let beta = 1 - alphaBarT / prevAlphaBar;
      beta = Math.min(Math.max(beta, 1e-8), 0.999); // clip, as in the original paper
      betas.push(beta);
      prevAlphaBar = alphaBarT;
    }
...
export function trainStep(
  ...
  for (const g of gradsAccum) {
    for (const row of g.dW) for (let i = 0; i < row.length; i++) { row[i] /= B; sumSq += row[i] * row[i]; }
    for (let i = 0; i < g.db.length; i++) { g.db[i] /= B; sumSq += g.db[i] * g.db[i]; }
  }
  // Global L2 gradient-norm clipping. This is the one intervention that
  // consistently made training visibly more stable in testing — without it,
  // the tiny hand-rolled optimizer occasionally takes an outsized step from
  // an unlucky (x, t, noise) sample, which shows up as noisy, non-monotonic
  // convergence. It's not a fix for undercapacity, just for instability.
  const gradNorm = Math.sqrt(sumSq);
  if (gradNorm > GRAD_CLIP_NORM) {
    const scale = GRAD_CLIP_NORM / gradNorm;
    for (const g of gradsAccum) {
      for (const row of g.dW) for (let i = 0; i < row.length; i++) row[i] *= scale;
      for (let i = 0; i < g.db.length; i++) g.db[i] *= scale;
    }
  }
...
// DDIM sampling (Song, Meng & Ermon, 2020) with sigma_t = 0: fully
// deterministic given the same starting noise, no per-step stochastic
// injection. Same trained network, same seed, same step count as the
// ancestral sampler above — the only thing that differs is the update rule,
// which is the whole point of exposing both as a toggle.
export function reverseSampleTrajectoryDDIM(
  mlp: MLP,
  sched: Schedule,
  timeDim: number,
  numPoints: number,
  ...
  for (let t = sched.T; t >= 1; t--) {
    const alphaBarT = sched.alphaBars[t - 1];
    const alphaBarPrev = t - 1 >= 1 ? sched.alphaBars[t - 2] : 1; // alphaBar_0 = 1 by convention
    const emb = timeEmbedding(t, timeDim);
    const sqrtAbT = Math.sqrt(alphaBarT);
    const sqrt1mAbT = Math.sqrt(1 - alphaBarT);
    const sqrtAbPrev = Math.sqrt(alphaBarPrev);
    const sqrt1mAbPrev = Math.sqrt(1 - alphaBarPrev);
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      const { acts } = forward(mlp, input);
      const predNoise = acts[acts.length - 1];
      // predicted x0, then re-noise it to the previous (lower) noise level
      const x0x = (p[0] - sqrt1mAbT * predNoise[0]) / sqrtAbT;
      const x0y = (p[1] - sqrt1mAbT * predNoise[1]) / sqrtAbT;
      next[i] = [
        sqrtAbPrev * x0x + sqrt1mAbPrev * predNoise[0],
        sqrtAbPrev * x0y + sqrt1mAbPrev * predNoise[1],
      ];
    }
    points = next;
    trajectory.push(points.map(p => p.slice()));
...
// Sliced Wasserstein distance: a cheap, well-behaved approximation to true
// Wasserstein distance in 2D. Projects both point sets onto random 1D
// directions, computes exact 1D Wasserstein distance on each projection
// (just a sort + mean absolute difference), and averages over directions.
export function slicedWasserstein(
  ...
  for (let p = 0; p < numProjections; p++) {
    ...
    const projA = a.slice(0, n).map(pt => pt[0] * ux + pt[1] * uy).sort((x, y) => x - y);
    const projB = b.slice(0, n).map(pt => pt[0] * ux + pt[1] * uy).sort((x, y) => x - y);
    let sum = 0;
    for (let i = 0; i < n; i++) sum += Math.abs(projA[i] - projB[i]);
    total += sum / n;
  }
